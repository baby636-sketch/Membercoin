...
namespace patternsearch
{
	...
	void static aesSearch(char *mainMemoryPsuedoRandomData, int threadNumber, int totalThreads, std::vector< std::pair<uint32_t,uint32_t> > *results, boost::mutex *mtx){
		...
		for(int k=startLoc;k<startLoc+searchNumber;k++){
			...
			for(int j=0;j<AES_ITERATIONS;j++){
				...
				//memcpy((char*)&cacheMemoryOperatingData2[0], (char*)&cacheMemoryOperatingData[0], cacheMemorySize );
			}
    ...
    bool pattern_verify( uint256 midHash, uint32_t a, uint32_t b ){
		...
		unsigned char *cacheMemoryOperatingData;
		unsigned char *cacheMemoryOperatingData2;	
		cacheMemoryOperatingData=new unsigned char[cacheMemorySize+16]; //4096 bytes + 16
		cacheMemoryOperatingData2=new unsigned char[cacheMemorySize]; //4096 bytes
		uint32_t* cacheMemoryOperatingData32 = (uint32_t*)cacheMemoryOperatingData;
		uint32_t* cacheMemoryOperatingData322 = (uint32_t*)cacheMemoryOperatingData2;
		...
		uint32_t startLocation=a*cacheMemorySize/chunkSize; //4096 bytes / 64 = 64 (a must be 0 to 63)
		uint32_t finishLocation=startLocation+(cacheMemorySize/chunkSize); // startlocation + 64
        ...
        if(solution<10000 && proofOfCalculation==b){
			return true;
		}
